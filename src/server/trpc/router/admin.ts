...
export const adminRouter = router({
    login: publicProcedure.input(z.object({ email: z.string().email(), password: z.string() }))
        .mutation(async ({ ctx, input }) => {
            ...
            if (email === process.env.ADMIN_EMAIL && password === process.env.ADMIN_PASSWORD) {
                ...
                const token = await new SignJWT({}).setProtectedHeader({ alg: "HS256" }).setJti(nanoid()).setIssuedAt()
    ...
    createPresignedUrl: adminProcedure.input(z.object({ fileType: z.string() })).mutation(async ({ input }) => {
        ...
        const { url, fields } = (await new Promise((resolve, reject) => {
            s3.createPresignedPost(
                {
                    Bucket: 'booking-software-avektor',
                    Fields: { key },
                    Expires: 60,
                    Conditions: [
    ...
    deleteMenuItem: adminProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            await s3.deleteObject({ Bucket: 'booking-software-avektor', Key: imageKey }).promise()
